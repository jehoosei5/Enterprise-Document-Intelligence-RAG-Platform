...
export default function ChatPage() {
  ...
  const [isHistoryOpen, setIsHistoryOpen] = useState(Boolean(routeConversationId))
  ...
  useEffect(() => {
    ...
    async function restore() {
      ...
      // Already showing this thread (e.g. just created and URL-updated).
      if (routeConversationId === loadedIdRef.current) {
        return
      }
      ...
      setLoadError(null)
      try {
        const detail = await getConversation(token, routeConversationId)
        if (cancelled) return
        setTurns(
          detail.turns.map((t) => ({
            id: t.query_id,
        ...
        loadedIdRef.current = routeConversationId
  ...
  const handleSelectConversation = (id: string) => {
    if (id === routeConversationId || loadingConversation) return
    navigate(`/chat/${id}`)
  }
    ...
    try {
      const done = await askQueryStream(
        ...
        (delta) => {
          setTurns((prev) =>
            prev.map((t) =>
              t.id === newTurn.id ? { ...t, answer: t.answer + delta, status: 'streaming' } : t,
            ),
          )
        },
      )
      if (!conversationId && done.conversation_id) {
        loadedIdRef.current = done.conversation_id
        setConversationId(done.conversation_id)
        navigate(`/chat/${done.conversation_id}`, { replace: true })
      }
      setTurns((prev) =>
        prev.map((t) =>
          t.id === newTurn.id
              ...
              <div className="flex flex-1 min-w-0 flex-col h-full">
                                ...
                                <div className="rounded-2xl rounded-tl-none bg-slate-50 px-6 py-5 text-sm text-slate-800 shadow-sm">
                                  {turn.status === 'loading' && !turn.answer ? (
                                    ...
                                    <div className="space-y-4">
                                      <MarkdownAnswer
                                        content={turn.answer}
                                        className="prose prose-sm prose-slate max-w-none leading-relaxed prose-p:my-2 prose-ul:my-2 prose-li:my-0.5"
                                      />
                ...
                <div className="mt-6 shrink-0">
                  ...
                  <div className="relative">
                    ...
                    <button
                      onClick={handleSend}
                      disabled={asking || loadingConversation || !input.trim()}
